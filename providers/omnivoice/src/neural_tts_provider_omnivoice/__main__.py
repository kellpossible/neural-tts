...
from .provider import OmnivoiceProvider

MAX_FRAME_BYTES = 16 * 1024 * 1024

log = logging.getLogger("neural_tts_provider_omnivoice")
...
async def _serve(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, *, eager: bool) -> None:
    provider = OmnivoiceProvider(eager_startup=eager)
    try:
        while True:
            try:
...
async def _handle_warmup(provider: OmnivoiceProvider) -> pb.Response:
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="neural-tts-provider-omnivoice")
    parser.add_argument(
        "--eager-startup",
        action="store_true",
