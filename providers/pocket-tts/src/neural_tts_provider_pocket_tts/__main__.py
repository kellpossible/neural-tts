...
Adopts the inherited socketpair on the FD named by NEURAL_TTS_PROVIDER_FD (set
by the daemon), runs a framed protobuf request/response loop until shutdown.
"""
...
log = logging.getLogger("neural_tts_provider_pocket_tts")
...
async def _serve(reader: asyncio.StreamReader, writer: asyncio.StreamWriter, *, eager: bool) -> None:
    provider = PocketTtsProvider(eager_startup=eager)
    try:
        while True:
            try:
...
async def _handle_warmup(provider: PocketTtsProvider) -> pb.Response:
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="neural-tts-provider-pocket-tts")
    parser.add_argument(
        "--eager-startup",
        action="store_true",
        help="Load the pocket-tts model at process start "
