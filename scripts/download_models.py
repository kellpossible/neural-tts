...
QWEN3_TTS_HF_REPO = "Qwen/Qwen3-TTS-12Hz-0.6B-Base"
QWEN3_TTS_DIR_NAME = "qwen3-tts-0.6b"


def _fetch_qwen3_tts(dest_dir: Path) -> None:
    """Snapshot-download the Qwen3-TTS model via the provider's venv."""
    venv_python = repo_root() / "providers" / "qwen3-tts" / ".venv" / "bin" / "python"
    if not venv_python.exists():
        die(
            f"qwen3-tts venv python not found at {venv_python}; "
            "run `mise run install-provider qwen3-tts --skip-models` first"
        )
